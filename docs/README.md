...
| [`W13_header_bands.md`](W13_header_bands.md) | **The worst defect this project has had.** Three health tables published 2,976 rows under the wrong disease names, and verification reported no mismatches throughout, correctly |
...
| | |
|---|---|
| [`D19_preface.md`](D19_preface.md) | The compilers' own preface and compilation notes. It answered most of the questions that were open at the time, including what a printed `0` means |
...
| [`T10_findings.md`](T10_findings.md), [`T11_findings.md`](T11_findings.md) | **Why it was dropped.** The novelty audit and the discovery that NTU's digital humanities centre already occupies the ground |
...
| [`D07_findings.md`](D07_findings.md) | Why a gap remains anyway: education, health and relief were of little interest to economists, so nobody digitised them |
...
[`TASKS.md`](TASKS.md) is the original task list, sixty-one items in the end,
with each marked done, dropped or stuck. The `T` numbers and `D01` to `D11` refer
to it; `D12` onwards were written after the list stopped being kept, and the `W`
and `R` numbers come from [`WORK.md`](WORK.md). It is the quickest way to see
what was tried and what was dropped.
